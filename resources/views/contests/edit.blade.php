        ...
        <div class="border-t border-gray-200 px-4 py-5 sm:p-6">
            <form action="{{ route('contests.update', $contest) }}" method="POST">
                ...
                <div class="grid grid-cols-1 gap-6">
                    <div>
                        <label for="title" class="block text-sm font-medium text-gray-700">Название конкурса</label>
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="5"
                                  class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md @error('description') border-red-500 @enderror">{{ old('description', $contest->description) }}</textarea>
                        @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        <label for="deadline_at" class="block text-sm font-medium text-gray-700">Дедлайн</label>
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $contest->is_active) ? 'checked' : '' }}
                ...
                <div class="mt-6 flex items-center justify-end space-x-3">
                    <a href="{{ route('contests.show', $contest) }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 font-bold py-2 px-4 rounded">
                        Отмена
                    </a>
